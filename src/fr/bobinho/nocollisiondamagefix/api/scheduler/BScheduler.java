package fr.bobinho.nocollisiondamagefix.api.scheduler;
...
public final class BScheduler {
    ...
    public synchronized int run(@Nonnull Runnable runnable) throws IllegalArgumentException {
        ...
        if (type == Type.SYNC) {
            if (getRepeatingDelay() != 0) {
                bukkitTaskId = Bukkit.getScheduler().runTaskTimer(NoCollisionDamageFixCore.getInstance(), runnable, getDelay(), getRepeatingDelay()).getTaskId();

            } else if (getDelay() != 0) {
                bukkitTaskId = Bukkit.getScheduler().runTaskLater(NoCollisionDamageFixCore.getInstance(), runnable, getDelay()).getTaskId();

            } else {
                bukkitTaskId = Bukkit.getScheduler().runTask(NoCollisionDamageFixCore.getInstance(), runnable).getTaskId();
            }

        } else {
            if (getRepeatingDelay() != 0) {
                bukkitTaskId = Bukkit.getScheduler().runTaskTimerAsynchronously(NoCollisionDamageFixCore.getInstance(), runnable, getDelay(), getRepeatingDelay()).getTaskId();
            } else if (getDelay() != 0) {
                bukkitTaskId = Bukkit.getScheduler().runTaskLaterAsynchronously(NoCollisionDamageFixCore.getInstance(), runnable, getDelay()).getTaskId();
            } else {
                bukkitTaskId = Bukkit.getScheduler().runTaskAsynchronously(NoCollisionDamageFixCore.getInstance(), runnable).getTaskId();
            }
        }
    ...
    public synchronized void run(@Nonnull Consumer<BukkitTask> task) throws IllegalArgumentException {
        ...
        if (type == Type.SYNC) {
            if (getRepeatingDelay() != 0) {
                Bukkit.getScheduler().runTaskTimer(NoCollisionDamageFixCore.getInstance(), task, getDelay(), getRepeatingDelay());
            } else if (getDelay() != 0) {
                Bukkit.getScheduler().runTaskLater(NoCollisionDamageFixCore.getInstance(), task, getDelay());
            } else {
                Bukkit.getScheduler().runTask(NoCollisionDamageFixCore.getInstance(), task);
            }

        } else {
            if (getRepeatingDelay() != 0) {
                Bukkit.getScheduler().runTaskTimerAsynchronously(NoCollisionDamageFixCore.getInstance(), task, getDelay(), getRepeatingDelay());
            } else if (getDelay() != 0) {
                Bukkit.getScheduler().runTaskLaterAsynchronously(NoCollisionDamageFixCore.getInstance(), task, getDelay());
            } else {
                Bukkit.getScheduler().runTaskAsynchronously(NoCollisionDamageFixCore.getInstance(), task);
            }
        }
    }
